...
typedef struct
{
  ...
  int fd;         /* For which connection was the event? */
} EventQueue;

/*}}}  */
/*{{{  globals */

char atb_tool_id[] = "$Id: atb-tool.c,v 1.7.2.1 2004/10/11 13:04:31 uid507 Exp $";
...
void ATBpostEvent(int fd, ATerm event)
{
  ...
  for(i=0, free_index=-1; i<MAX_NR_QUEUES; i++) {
    /* Hier heb ik een extra conditie aan toegevoegd, zodat een event
     * alleen aan een queue wordt toegevoegd van de juiste fd:
     */
    if(event_queues[i].afun == afun && event_queues[i].fd == fd) {
      break;
    }
  ...
  if (i >= MAX_NR_QUEUES) {
    ...
    /* Hier sla ik de fd bij de queue op: */
    event_queues[i].fd = fd;
    ATprotectAFun(afun);
...
static void handle_ack_event(int fd, AFun afun)
{
  ...
  for(i=MAX_NR_QUEUES-1; i>=0; i--) {
    /* De extra conditie zorgt ervoor dat alleen de queues voor de
     * huidige fd in aanmerking komen. De rest blijft even staan totdat
     * de ack-events binnenkomen voor hun fd:
     */
    if(event_queues[i].afun == afun && event_queues[i].fd == fd) {
